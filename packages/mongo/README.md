...
The MongoDB layer of the monorepo: the driver, connection handling tuned for Cloudflare Workers, a local database for development, and one example collection (`notes`) showing the shape every collection should follow.
...
| `@repo/mongo/notes` | `listNotes`, `createNote`, `ensureNoteIndexes` | Server only |
| `@repo/mongo/shared` | `Note` type, `NOTE_TEXT_MAX_LENGTH` | Anywhere, including React components |
...
const notes = await withDb(uri, (db) => listNotes(db));
const note = await withDb(uri, (db) => createNote(db, { text: "hello" }));
```
...
`src/notes.ts` is the template for new collections:
